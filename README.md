<div align="center">

<img src="https://github.com/SP-XD/SP-XD/blob/main/images/hellocoders_rounded.gif?raw=true" width="60%"/> <br>
<img src="https://github.com/SP-XD/SP-XD/blob/main/images/dev-working_rounded.gif?raw=true" width="40%"/><br>

<h2>Hi 👋 I'm Ahmed Hisham</h2>
<h4>Android Mobile Application Developer | Front-End Developer | Database</h4>

</div>

---

👨‍💻 About Me
- 📱 Android Mobile Application Developer using **Kotlin & Android Studio**
- 🌐 Front-End Developer (**HTML, CSS, JavaScript**)
- 🗄️ Database Experience: **Oracle Database & MySQL**
- 🎓 Computer Engineering Student
- 🚀 Passionate about building real-world and scalable applications

---

🚀 Programming Languages
![Kotlin](https://img.shields.io/badge/Kotlin-0095D5?style=flat&logo=kotlin&logoColor=white)
![C++](https://img.shields.io/badge/C%2B%2B-00599C?style=flat&logo=c%2B%2B&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-323330?style=flat&logo=javascript&logoColor=F7DF1E)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?style=flat&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat&logo=css3&logoColor=white)

---

📱 Mobile Development
![Android](https://img.shields.io/badge/Android-3DDC84?style=flat&logo=android&logoColor=white)
![Android Studio](https://img.shields.io/badge/Android_Studio-3DDC84?style=flat&logo=android-studio&logoColor=white)

---

🗄️ Databases
![Oracle](https://img.shields.io/badge/Oracle-F80000?style=flat&logo=oracle&logoColor=white)
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat&logo=mysql&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-ffca28?style=flat&logo=firebase&logoColor=black)
![SQLite](https://img.shields.io/badge/SQLite-07405E?style=flat&logo=sqlite&logoColor=white)

---

🛠️ Tools
![Git](https://img.shields.io/badge/GIT-E44C30?style=flat&logo=git&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-0078D4?style=flat&logo=visual-studio-code&logoColor=white)
![GitHub Pages](https://img.shields.io/badge/GitHub_Pages-222222?style=flat&logo=github-pages&logoColor=white)

---

🧠 Skills Summary (Code Style)

```kotlin
class AhmedHisham {
    val skills = mapOf(
        "Programming Languages" to listOf("Kotlin", "C++", "JavaScript", "HTML", "CSS"),
        "Mobile Development" to listOf("Android", "Android Studio"),
        "Databases" to listOf("Oracle Database", "MySQL", "Firebase", "SQLite"),
        "Tools" to listOf("Git", "VS Code"),
        "Operating Systems" to listOf("Windows", "Linux")
    )
}
